...
PARAM_SPECS: dict[str, dict[str, tuple]] = {
    "drone": {
        ...
        # freq_norm: 0〜1 → SC側で 20〜300Hz に対数マップ（音域を大きく変える）
        "freq_norm":    (0.0,  1.0,  0.20, "/matoma/drone/param"),
        # brightness: 0〜1 → SC側で 200〜8000Hz カットオフ（音色の明暗）
        "brightness":   (0.0,  1.0,  0.50, "/matoma/drone/param"),
    },
    "granular": {
    ...
    "gran_synth": {
        ...
        "spread":    (0.0,  1.0,  0.20, "/matoma/gran_synth/param"),
        "bright":    (0.0,  1.0,  0.40, "/matoma/gran_synth/param"),
        "harm":      (0.0,  1.0,  0.30, "/matoma/gran_synth/param"),
        ...
        # freq_norm: 0〜1 → SC側で 55〜880Hz（A1〜A5）に対数マップ
        "freq_norm": (0.0,  1.0,  0.30, "/matoma/gran_synth/param"),
    },
    "gran_sampler": {
    ...
    # pad_synth: サブトラクティブ・パッドシンセ（鋸波+パルス → レゾナントフィルター）
    "pad_synth": {
        "cutoff":    (80.0, 8000.0, 600.0, "/matoma/pad_synth/param"),
        "reson":     (0.0,  1.0,    0.25,  "/matoma/pad_synth/param"),
        "detune":    (0.0,  1.0,    0.15,  "/matoma/pad_synth/param"),
        "lfo_rate":  (0.0,  1.0,    0.25,  "/matoma/pad_synth/param"),
        "lfo_depth": (0.0,  1.0,    0.30,  "/matoma/pad_synth/param"),
        "room":      (0.0,  1.0,    0.60,  "/matoma/pad_synth/param"),
        "amp":       (0.0,  1.0,    0.30,  "/matoma/pad_synth/param"),
    },
...
STATE_ZONES: dict[str, dict[str, dict[str, dict[str, float]]]] = {
    "void": {
        "drone": {
            "feedback_amt": {"center": 0.08, "width": 0.04},
            "shimmer":      {"center": 0.05, "width": 0.04},
            "room":         {"center": 0.85, "width": 0.06},
            "amp":          {"center": 0.12, "width": 0.03},
            # 超低域（≈20〜30Hz）・暗い音色
            "freq_norm":    {"center": 0.05, "width": 0.03},
            "brightness":   {"center": 0.08, "width": 0.04},
        },
        "granular": {
            "density": {"center":  4.0, "width": 2.5},
            "spray":   {"center": 0.08, "width": 0.05},
            "pos":     {"center": 0.50, "width": 0.12},
            "room":    {"center": 0.85, "width": 0.06},
        },
        "gran_synth": {
            "density":   {"center":  7.0, "width": 3.0},
            "grainDur":  {"center": 0.35, "width": 0.12},
            "spread":    {"center": 0.03, "width": 0.02},
            "bright":    {"center": 0.08, "width": 0.04},
            "harm":      {"center": 0.04, "width": 0.02},
            "chaos":     {"center": 0.04, "width": 0.03},
            "room":      {"center": 0.92, "width": 0.05},
            "amp":       {"center": 0.08, "width": 0.03},
            # void: 低域（A1〜A2付近 ≈55〜80Hz）
            "freq_norm": {"center": 0.08, "width": 0.04},
        },
        "gran_sampler": {
            "pos":     {"center": 0.50, "width": 0.12},
            "density": {"center":  4.0, "width": 2.5},
        ...
        "pad_synth": {
            "cutoff":    {"center": 120.0, "width": 40.0},
            "reson":     {"center": 0.10,  "width": 0.05},
            "detune":    {"center": 0.04,  "width": 0.02},
            "lfo_rate":  {"center": 0.06,  "width": 0.03},
            "lfo_depth": {"center": 0.08,  "width": 0.04},
            "room":      {"center": 0.90,  "width": 0.05},
            "amp":       {"center": 0.08,  "width": 0.03},
        },
    ...
    "sparse": {
        "drone": {
            ...
            "shimmer":      {"center": 0.20, "width": 0.10},
            "room":         {"center": 0.75, "width": 0.10},
            "amp":          {"center": 0.22, "width": 0.06},
            # 低域（≈40〜70Hz）・少し明るめ
            "freq_norm":    {"center": 0.18, "width": 0.07},
            "brightness":   {"center": 0.22, "width": 0.09},
        },
        "granular": {
            "density": {"center":  9.0, "width": 4.0},
            "spray":   {"center": 0.25, "width": 0.12},
            "pos":     {"center": 0.50, "width": 0.18},
            "room":    {"center": 0.65, "width": 0.10},
        },
        "gran_synth": {
            "density":   {"center": 14.0, "width": 6.0},
            "grainDur":  {"center": 0.25, "width": 0.10},
            "spread":    {"center": 0.18, "width": 0.08},
            "bright":    {"center": 0.25, "width": 0.10},
            "harm":      {"center": 0.15, "width": 0.07},
            "chaos":     {"center": 0.12, "width": 0.07},
            "room":      {"center": 0.75, "width": 0.08},
            "amp":       {"center": 0.18, "width": 0.06},
            # sparse: A1〜A2付近（55〜100Hz）
            "freq_norm": {"center": 0.18, "width": 0.08},
        },
        "gran_sampler": {
            "pos":     {"center": 0.50, "width": 0.18},
            "density": {"center":  9.0, "width": 4.0},
            "spray":   {"center": 0.18, "width": 0.08},
            "room":    {"center": 0.72, "width": 0.08},
            "amp":     {"center": 0.18, "width": 0.06},
        },
        "pad_synth": {
            "cutoff":    {"center": 280.0, "width": 100.0},
            "reson":     {"center": 0.18,  "width": 0.08},
            "detune":    {"center": 0.10,  "width": 0.05},
            "lfo_rate":  {"center": 0.15,  "width": 0.07},
            "lfo_depth": {"center": 0.18,  "width": 0.08},
            "room":      {"center": 0.75,  "width": 0.08},
            "amp":       {"center": 0.16,  "width": 0.05},
        },
    ...
    "medium": {
        "drone": {
            "feedback_amt": {"center": 0.28, "width": 0.12},
            "shimmer":      {"center": 0.42, "width": 0.16},
            "room":         {"center": 0.68, "width": 0.12},
            "amp":          {"center": 0.38, "width": 0.07},
            # 中低域（≈60〜100Hz）・中程度の明るさ
            "freq_norm":    {"center": 0.32, "width": 0.12},
            "brightness":   {"center": 0.48, "width": 0.15},
        },
        "granular": {
            "density": {"center": 15.0, "width": 6.0},
            "spray":   {"center": 0.48, "width": 0.18},
            "pos":     {"center": 0.50, "width": 0.25},
            "room":    {"center": 0.52, "width": 0.12},
        },
        "gran_synth": {
            "density":   {"center": 28.0, "width": 10.0},
            "grainDur":  {"center": 0.18, "width": 0.08},
            "spread":    {"center": 0.38, "width": 0.14},
            "bright":    {"center": 0.42, "width": 0.15},
            "harm":      {"center": 0.38, "width": 0.14},
            "chaos":     {"center": 0.32, "width": 0.14},
            "room":      {"center": 0.60, "width": 0.10},
            "amp":       {"center": 0.40, "width": 0.07},
            # medium: A2〜A3付近（110〜220Hz）
            "freq_norm": {"center": 0.35, "width": 0.12},
        },
        "gran_sampler": {
            "pos":     {"center": 0.50, "width": 0.25},
            "density": {"center": 20.0, "width": 8.0},
            "spray":   {"center": 0.30, "width": 0.12},
            "room":    {"center": 0.52, "width": 0.10},
            "amp":     {"center": 0.48, "width": 0.07},
        },
        "pad_synth": {
            "cutoff":    {"center": 700.0, "width": 250.0},
            "reson":     {"center": 0.28,  "width": 0.12},
            "detune":    {"center": 0.20,  "width": 0.08},
            "lfo_rate":  {"center": 0.28,  "width": 0.12},
            "lfo_depth": {"center": 0.35,  "width": 0.14},
            "room":      {"center": 0.60,  "width": 0.10},
            "amp":       {"center": 0.28,  "width": 0.07},
        },
    ...
    "dense": {
        "drone": {
            "feedback_amt": {"center": 0.42, "width": 0.15},
            "shimmer":      {"center": 0.65, "width": 0.20},
            "room":         {"center": 0.58, "width": 0.12},
            "amp":          {"center": 0.55, "width": 0.07},
            # 中域（≈80〜160Hz）・明るめ
            "freq_norm":    {"center": 0.50, "width": 0.15},
            "brightness":   {"center": 0.70, "width": 0.18},
        },
        "granular": {
            "density": {"center": 26.0, "width": 9.0},
            "spray":   {"center": 0.62, "width": 0.20},
            "pos":     {"center": 0.50, "width": 0.30},
            "room":    {"center": 0.40, "width": 0.12},
        },
        "gran_synth": {
            "density":   {"center": 44.0, "width": 13.0},
            "grainDur":  {"center": 0.12, "width": 0.06},
            "spread":    {"center": 0.60, "width": 0.18},
            "bright":    {"center": 0.65, "width": 0.18},
            "harm":      {"center": 0.65, "width": 0.18},
            "chaos":     {"center": 0.55, "width": 0.18},
            "room":      {"center": 0.40, "width": 0.10},
            "amp":       {"center": 0.52, "width": 0.07},
            # dense: A3〜A4付近（220〜440Hz）
            "freq_norm": {"center": 0.55, "width": 0.15},
        },
        "gran_sampler": {
            "pos":     {"center": 0.50, "width": 0.30},
            "density": {"center": 36.0, "width": 11.0},
            "spray":   {"center": 0.52, "width": 0.16},
            "room":    {"center": 0.40, "width": 0.10},
            "amp":     {"center": 0.58, "width": 0.07},
        },
        "pad_synth": {
            "cutoff":    {"center": 2000.0, "width": 700.0},
            "reson":     {"center": 0.45,   "width": 0.18},
            "detune":    {"center": 0.38,   "width": 0.14},
            "lfo_rate":  {"center": 0.50,   "width": 0.18},
            "lfo_depth": {"center": 0.55,   "width": 0.18},
            "room":      {"center": 0.42,   "width": 0.10},
            "amp":       {"center": 0.45,   "width": 0.08},
        },
    ...
    "intense": {
        "drone": {
            "feedback_amt": {"center": 0.62, "width": 0.18},
            "shimmer":      {"center": 0.88, "width": 0.10},
            "room":         {"center": 0.45, "width": 0.12},
            "amp":          {"center": 0.72, "width": 0.08},
            # 高域（≈120〜250Hz）・最も明るい
            "freq_norm":    {"center": 0.68, "width": 0.18},
            "brightness":   {"center": 0.90, "width": 0.08},
        },
        "granular": {
            "density": {"center": 36.0, "width": 11.0},
            "spray":   {"center": 0.75, "width": 0.18},
            "pos":     {"center": 0.50, "width": 0.35},
            "room":    {"center": 0.28, "width": 0.10},
        },
        "gran_synth": {
            "density":   {"center": 58.0, "width": 16.0},
            "grainDur":  {"center": 0.06, "width": 0.04},
            "spread":    {"center": 0.88, "width": 0.10},
            "bright":    {"center": 0.88, "width": 0.10},
            "harm":      {"center": 0.88, "width": 0.10},
            "chaos":     {"center": 0.82, "width": 0.14},
            "room":      {"center": 0.28, "width": 0.08},
            "amp":       {"center": 0.68, "width": 0.08},
            # intense: A4〜A5付近（440〜880Hz）
            "freq_norm": {"center": 0.75, "width": 0.18},
        },
        "gran_sampler": {
            "pos":     {"center": 0.50, "width": 0.35},
            "density": {"center": 52.0, "width": 13.0},
            "spray":   {"center": 0.72, "width": 0.18},
            "room":    {"center": 0.28, "width": 0.08},
            "amp":     {"center": 0.72, "width": 0.08},
        },
        "pad_synth": {
            "cutoff":    {"center": 5500.0, "width": 1500.0},
            "reson":     {"center": 0.70,   "width": 0.18},
            "detune":    {"center": 0.65,   "width": 0.18},
            "lfo_rate":  {"center": 0.78,   "width": 0.15},
            "lfo_depth": {"center": 0.80,   "width": 0.14},
            "room":      {"center": 0.28,   "width": 0.08},
            "amp":       {"center": 0.68,   "width": 0.08},
        },
